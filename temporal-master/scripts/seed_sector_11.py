"""
seed_sector_11.py — Sector 11: El Ascenso del Arquitecto (Nexo Central)
========================================================================
...
Uso standalone:
    python -m scripts.seed_sector_11
    python -m scripts.seed_sector_11 --dry-run
"""
        ...
        try:
            if not dry_run:
                await session.execute(
                    delete(Challenge).where(Challenge.sector_id == 11)
                )
            for data in SECTOR_11:
            ...
            if not dry_run:
                ...
                print(f"\n  💾  Commit OK — {len(SECTOR_11)} misiones insertadas.")
