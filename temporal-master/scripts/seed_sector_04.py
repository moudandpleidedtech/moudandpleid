"""
Seed — SECTOR 04: Arquitectura de Datos (10 niveles, IDs 31–40).
...
async def seed() -> None:
    ...
    async with SessionLocal() as session:
        deleted = await session.execute(
            delete(Challenge).where(Challenge.sector_id == 4)
        )
        deleted_count = deleted.rowcount
        await session.flush()
        print(f"🧹  Sector 04 anterior eliminado — {deleted_count} challenge(s) removidos.")

        print(f"\n🌱  Insertando {len(SECTOR_04)} niveles del Sector 04...\n")
        for data in SECTOR_04:
    ...
    print(f"\n✅  Sector 04 cargado — {len(SECTOR_04)} niveles listos.")
